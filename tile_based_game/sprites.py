...
def collide_with_wall(sprite, group, direction):
    if direction == 'x':
...
class Mob(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if target_dist.length_squared() < DETECT_RADIUS**2:
            if random() < 0.002:
                choice(self.game.zombie_maon_sounds).play()
            self.rot = target_dist.angle_to(vec(1, 0)) % 360
...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, game, pos, direction, damage):
        ...
        pygame.sprite.Sprite.__init__(self, self.groups)
        self.game = game
        self.image = game.bullet_imgs[WEAPONS[game.player.weapon]['bullet_size']]
        self.rect = self.image.get_rect()
...
class MuzFlash(pygame.sprite.Sprite):
    ...
    def update(self):
        if pygame.time.get_ticks() - self.spawn_time > MUZZLE_DURATION:
            self.kill()
